...
export default function AddAttendancePage() {
  ...
  const loadEmployees = async () => {
    try {
      ...
      initializeAttendanceData(usersData);
    } catch (error) {
  ...
  const initializeAttendanceData = (users: User[]) => {
    const data: Record<string, Record<string, string>> = {};
    users.forEach((employee) => {
      data[employee.uId] = {};
      for (let day = 1; day <= daysInMonth; day++) {
        const dateStr = `${currentMonth}-${String(day).padStart(2, '0')}`;
        data[employee.uId][dateStr] = 'absent';
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    if (user) {
      ...
      const canAdd = currentUserDoc?.accessLevelMap?.attendence || 
    ...
    try {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Add employee attendance for this day
  ...
  const handleMonthChange = (month: string) => {
    ...
    // This would need to be implemented to load existing data for different months
  };
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        {/* <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Add New Attendance</h1>
            <p className="text-muted-foreground">
              Mark attendance for all employees for the selected month
            </p>
          </div>
        </div> */}
        {/* Right controls removed from header; moved next to month filter */}
      </div>

      {/* Content */}
      <ClientOnly>
        <div className="space-y-4">
          {/* Month Selector + Actions (left-aligned) */}
