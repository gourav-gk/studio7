...
export default function ProjectShootsPage() {
  ...
  const allShoots: ShootRow[] = useMemo(() =>
    data.flatMap(project =>
      (Array.isArray(project.shoots) ? project.shoots : []).map(shoot => ({
        ...shoot,
        projectName: project.projectName,
        projectId: project.projectId,
        assignedEmployees: (shoot as any).assignedEmployees || [],
      }) as ShootRow)
    ),
    [data]
  );
  ...
  const handleSaveAssign = async () => {
    ...
    try {
      // Find the project and update the shoot's assignedEmployees
      const projectDocRef = doc(firestore, "projects", selectedShoot.projectId);
      // Fetch the project document
      // (Assume the shoots array is not too large for a single update)
